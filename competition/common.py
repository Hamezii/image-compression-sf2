...
# Our optimal strategy is to use LBT at 8x8 transform size under custom Huffman table
...
def huffman_bits_gap(step, X, N, M):
    ...
    diff = total_bits - 40960.0 + 1424.0 + 1.0 + 8.0*8.0
    if diff > 0:
        diff = diff * 100000.0
    return abs(diff)

def calc_lbt_bit_size(step, X, N, M):
    vlc = jpegenc_lbt(X, step, N, M, opthuff=True, log=False)[0]
    return sum(vlc[:, 1])
...
def get_bit_size_and_rms(step, X, N, M):
    vlc, header = jpegenc(X, step, N, M, opthuff=True, log=False)[0]
    return sum(vlc[:, 1])

def get_best_quant(X, N, M):
    EPSILON = 8.0
    bits_max = 40960.0 - 1424.0 - 8.0*8.0 - EPSILON*2
    
    valid_quants = {}
    for quant in range(2, 80):
        size = calc_lbt_bit_size(lbt_quant, X, N, M)
